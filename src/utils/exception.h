...
unsigned char exception_handler(void* contextIn) {
	...
	#define PROGRAM_COUNTER 38
	#define LINK_REGISTER 35
	
	
	
	DisassemblePPCRange((void*)context[PROGRAM_COUNTER]-0x18, (void*)context[PROGRAM_COUNTER]+0x4, (void*)exception_disassembly_helper, 0, 0);
	
	char crashMessage[128];
	__os_snprintf(crashMessage, 128, "ya done goofed, nice");
